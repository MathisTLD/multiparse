// Inspired By
// https://github.com/fictivekin/fknpm-growable-uint8-array
...
const proxyHandler = {
  ...
  ownKeys(target) {
    const keys = new Set(
      Reflect.ownKeys(target.unwrap()).concat(Reflect.ownKeys(target))
    );
    keys.delete("buf");
 ...
 */
function GrowableUint8Array(buf = null, expansionRate = 2) {
  if (buf) {
...
module.exports = GrowableUint8Array;
...
/*
    Optimized passthrough methods that don't need unwrap to be called
*/

GrowableUint8Array.prototype.indexOf = function indexOf(
  searchElement,
  fromIndex = 0
) {
  if (!this.buf || fromIndex >= this.length) {
    return -1;
  } else {
    for (var i = fromIndex; i < this.length; i++) {
      if (this.buf[i] === searchElement) return i;
    }
    return -1;
  }
};

GrowableUint8Array.prototype.subarray = function subarray() {
  if (!this.buf) this.buf = new Uint8Array();
  return this.buf.subarray(...Array.from(arguments));
};
...
*/
const _WRAP_FNS = ["copyWithin", "filter", "map", "reverse", "slice", "sort"];
